...
class Storyboard {
	...
	parse(osb: string, createNew: boolean = true): Storyboard | undefined {
		...
		lines.forEach((line, index) => {
			...
			const isElementTitle =
				line.startsWith(ESbElementType.Sprite) ||
				line.startsWith(ESbElementType.Animation) ||
				line.startsWith(ESbElementType.Sample);
			...
			if (currentElement) {
				...
				if (isElementProperty && !isElementPropertyGroup) {
					...
					const isLastElementPropertyGroup =
						(typeof lines[index + 1] === "string" && !lines[index + 1].startsWith("  ")) || !lines[index + 1];
					...
					if (isLastElementPropertyGroup && currentPropertyType && currentPropertyTrimmedLine) {
						parseElementGroupProperty(
							currentPropertyTrimmedLine,
							currentPropertyType,
							currentElement,
							currentGroupElement
						);
	...
	toString(): string {
		return [
			...
			...Object.entries(this.getLayers()).map(([, layerData]) => {
				const { title, elements } = layerData ?? {};
				return [title, ...elements.map((element) => element.toString())].join("\n");
			})
		].join("\n");
	}
}
